...
import { updateInstruction } from 'services/checkout'
...
const validationSchema = Yup.object().shape({
	note: Yup.string().max(200, 'Not more than 200 characters'),
})
...
const OrderInstruction = ({
	order,
	onClose,
	title = 'Provide any instructions for our team.',
	...
	const onSubmit = async (values: { note: string }) => {
		...
		try {
			await updateInstruction(order._id, {
				note: values.note,
			})
				...
				{({ errors, touched, values }) => (
					<StyledForm>
						...
						<TextAreaContainer>
							...
							/>
							<Count>{values.note.length}/200</Count>
						</TextAreaContainer>
						{errors.note && touched.note && (
							<ErrorField error={errors.note} />
						)}
